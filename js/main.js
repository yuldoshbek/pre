...
function initPackagingSlider() {
    const slider = document.getElementById('packaging-slider');
    const track = slider?.querySelector('.packaging__track');
    const prevBtn = document.querySelector('.packaging__arrow--prev');
    const nextBtn = document.querySelector('.packaging__arrow--next');

    if (!slider || !track) return;

    const cards = track.querySelectorAll('.packaging__card');
    const cardWidth = 340 + 24; // Card width + gap
    let currentIndex = 0;
    let autoScrollInterval;
    let isDragging = false;
